...
func (t *brandModelTable) Setup(ctx context.Context, d sqldb.Executor) error {
	_, err := d.ExecContext(ctx, "CREATE TABLE IF NOT EXISTS "+t.TableName+" (creatorid VARCHAR(31), brandid VARCHAR(63), creation_time BIGINT NOT NULL, PRIMARY KEY (creatorid, brandid));")
	if err != nil {
		return err
	}
	return nil
...
func (t *brandModelTable) GetBrandModelByCreatorBrand(ctx context.Context, d sqldb.Executor, creatorid string, brandid string) (*BrandModel, error) {
	m := &BrandModel{}
...
func (t *brandModelTable) DelByCreatorBrand(ctx context.Context, d sqldb.Executor, creatorid string, brandid string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE creatorid = $1 AND brandid = $2;", creatorid, brandid)
	return err
}

func (t *brandModelTable) DelByCreatorBrands(ctx context.Context, d sqldb.Executor, creatorid string, brandids []string) error {
	paramCount := 1
...
func (t *brandModelTable) GetBrandModelByCreator(ctx context.Context, d sqldb.Executor, creatorid string, limit, offset int) (_ []BrandModel, retErr error) {
	res := make([]BrandModel, 0, limit)
	rows, err := d.QueryContext(ctx, "SELECT creatorid, brandid, creation_time FROM "+t.TableName+" WHERE creatorid = $3 ORDER BY brandid LIMIT $1 OFFSET $2;", limit, offset, creatorid)
	if err != nil {
		return nil, err
	}
	defer func() {
