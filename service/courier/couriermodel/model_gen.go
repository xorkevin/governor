...
func (t *linkModelTable) GetLinkModelEqLinkID(ctx context.Context, d sqldb.Executor, linkid string) (*LinkModel, error) {
	m := &LinkModel{}
...
func (t *linkModelTable) DelEqLinkID(ctx context.Context, d sqldb.Executor, linkid string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE linkid = $1;", linkid)
	return err
}

func (t *linkModelTable) DelHasLinkID(ctx context.Context, d sqldb.Executor, linkids []string) error {
	paramCount := 0
...
func (t *linkModelTable) GetLinkModelEqCreatorIDOrdCreationTime(ctx context.Context, d sqldb.Executor, creatorid string, orderasc bool, limit, offset int) (_ []LinkModel, retErr error) {
	order := "DESC"
	if orderasc {
		order = "ASC"
	}
...
func (t *brandModelTable) Setup(ctx context.Context, d sqldb.Executor) error {
...
func (t *brandModelTable) Insert(ctx context.Context, d sqldb.Executor, m *BrandModel) error {
...
func (t *brandModelTable) InsertBulk(ctx context.Context, d sqldb.Executor, models []*BrandModel, allowConflict bool) error {
	conflictSQL := ""
	if allowConflict {
		conflictSQL = " ON CONFLICT DO NOTHING"
	}
...
func (t *brandModelTable) GetBrandModelEqCreatorIDEqBrandID(ctx context.Context, d sqldb.Executor, creatorid string, brandid string) (*BrandModel, error) {
	m := &BrandModel{}
...
func (t *brandModelTable) DelEqCreatorIDEqBrandID(ctx context.Context, d sqldb.Executor, creatorid string, brandid string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE creatorid = $1 AND brandid = $2;", creatorid, brandid)
	return err
}

func (t *brandModelTable) DelEqCreatorIDHasBrandID(ctx context.Context, d sqldb.Executor, creatorid string, brandids []string) error {
	paramCount := 1
...
func (t *brandModelTable) GetBrandModelEqCreatorIDOrdCreationTime(ctx context.Context, d sqldb.Executor, creatorid string, orderasc bool, limit, offset int) (_ []BrandModel, retErr error) {
	order := "DESC"
	if orderasc {
		order = "ASC"
	}
