...
func (t *invModelTable) GetModelEqUseridEqRoleGtCreationTime(ctx context.Context, d sqldb.Executor, userid string, role string, creationtime int64) (*Model, error) {
	m := &Model{}
...
func (t *invModelTable) DelEqUseridEqRole(ctx context.Context, d sqldb.Executor, userid string, role string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE userid = $1 AND role = $2;", userid, role)
	return err
}

func (t *invModelTable) DelEqUseridHasRole(ctx context.Context, d sqldb.Executor, userid string, roles []string) error {
	paramCount := 1
...
func (t *invModelTable) DelEqRole(ctx context.Context, d sqldb.Executor, role string) error {
...
func (t *invModelTable) GetModelEqRoleGtCreationTimeOrdCreationTime(ctx context.Context, d sqldb.Executor, role string, creationtime int64, orderasc bool, limit, offset int) (_ []Model, retErr error) {
	order := "DESC"
	if orderasc {
		order = "ASC"
	}
