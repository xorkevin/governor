...
func (t *invModelTable) Setup(ctx context.Context, d sqldb.Executor) error {
	_, err := d.ExecContext(ctx, "CREATE TABLE IF NOT EXISTS "+t.TableName+" (userid VARCHAR(31), role VARCHAR(255), invited_by VARCHAR(31) NOT NULL, creation_time BIGINT NOT NULL, PRIMARY KEY (userid, role));")
	if err != nil {
		return err
	}
	...
	_, err = d.ExecContext(ctx, "CREATE INDEX IF NOT EXISTS "+t.TableName+"_role_creation_time_index ON "+t.TableName+" (role, creation_time);")
	if err != nil {
		return err
	}
	return nil
...
func (t *invModelTable) GetModelByUserRole(ctx context.Context, d sqldb.Executor, userid string, role string, creationtime int64) (*Model, error) {
	m := &Model{}
...
func (t *invModelTable) DelByUserRole(ctx context.Context, d sqldb.Executor, userid string, role string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE userid = $1 AND role = $2;", userid, role)
	return err
}

func (t *invModelTable) DelByUserRoles(ctx context.Context, d sqldb.Executor, userid string, roles []string) error {
	paramCount := 1
...
func (t *invModelTable) DelByRole(ctx context.Context, d sqldb.Executor, role string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE role = $1;", role)
	return err
}

func (t *invModelTable) GetModelByUserid(ctx context.Context, d sqldb.Executor, userid string, creationtime int64, limit, offset int) (_ []Model, retErr error) {
	res := make([]Model, 0, limit)
	rows, err := d.QueryContext(ctx, "SELECT userid, role, invited_by, creation_time FROM "+t.TableName+" WHERE userid = $3 AND creation_time > $4 ORDER BY creation_time DESC LIMIT $1 OFFSET $2;", limit, offset, userid, creationtime)
	if err != nil {
		return nil, err
	}
	defer func() {
...
func (t *invModelTable) GetModelByRole(ctx context.Context, d sqldb.Executor, role string, creationtime int64, limit, offset int) (_ []Model, retErr error) {
	res := make([]Model, 0, limit)
	rows, err := d.QueryContext(ctx, "SELECT userid, role, invited_by, creation_time FROM "+t.TableName+" WHERE role = $3 AND creation_time > $4 ORDER BY creation_time DESC LIMIT $1 OFFSET $2;", limit, offset, role, creationtime)
	if err != nil {
		return nil, err
	}
	defer func() {
