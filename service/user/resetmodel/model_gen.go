...
func (t *resetModelTable) DelEqUserid(ctx context.Context, d sqldb.Executor, userid string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE userid = $1;", userid)
	return err
}

func (t *resetModelTable) GetModelEqUseridEqKind(ctx context.Context, d sqldb.Executor, userid string, kind string) (*Model, error) {
	m := &Model{}
...
func (t *resetModelTable) UpdModelEqUseridEqKind(ctx context.Context, d sqldb.Executor, m *Model, userid string, kind string) error {
...
func (t *resetModelTable) DelEqUseridEqKind(ctx context.Context, d sqldb.Executor, userid string, kind string) error {
	_, err := d.ExecContext(ctx, "DELETE FROM "+t.TableName+" WHERE userid = $1 AND kind = $2;", userid, kind)
	return err
}

func (t *resetModelTable) DelLtCodeTime(ctx context.Context, d sqldb.Executor, codetime int64) error {
