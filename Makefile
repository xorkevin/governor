...
pg-run:
	docker run -d --name $(POSTGRES_CONTAINER) \
		...
		-e POSTGRES_PASSWORD=$(POSTGRES_PASS) postgres:alpine
...
minio-run:
	docker run -d --name $(MINIO_CONTAINER) \
		--network=$(DOCKER_NETWORK) -p 9000:9000 \
		-v $(MINIO_VOLUME):/export \
		-v $(MINIO_VOLUME_CONFIG):/root/.minio \
...
mail-run:
	docker run -d --name $(MAIL_CONTAINER) \
		--network=$(DOCKER_NETWORK) -p 1025:1025 -p 8025:8025 mailhog/mailhog
